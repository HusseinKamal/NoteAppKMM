    ...
    @MainActor class NoteDetailViewModel: ObservableObject {
        ...
        func saveNote(onSaved: @escaping () -> Void) {
            noteDataSource?.insertNote(
                note: Note(id: noteId == nil ? nil : KotlinLong(value: noteId!), title: self.noteTitle, content: self.noteContent, colorHex: self.noteColor as! Int64, created: DateTimeUtil().now()), completionHandler: { error in
                    onSaved()
                })
        }
